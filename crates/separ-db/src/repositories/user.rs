...
use separ_core::{
    Result, User, UserId, UserRepository, TenantId, SeparError,
};
...
impl UserRepository for PgUserRepository {
    ...
    #[instrument(skip(self))]
    async fn get_by_external_id(&self, _tenant_id: TenantId, _external_id: &str) -> Result<Option<User>> {
        Ok(None)
    }

    #[instrument(skip(self))]
    async fn list_by_tenant(&self, _tenant_id: TenantId, _offset: u32, _limit: u32) -> Result<Vec<User>> {
        Ok(vec![])
    }
